...
if [[ -z ${my_env} ]] ||
    ...
    echo ''
    echo 'Usage:'
    echo '    bash scripts/install-go.sh <env> <domain> <zone> <project-name>'
    echo ''
    echo 'Example:'
    echo "    bash scripts/install-go.sh 'dev' 'dev-123' 'example.com' 'foobar'"
    echo ''
    exit 1
fi
...
if [[ ! -e libreoffice-as-a-service ]]; then
    bash scripts/builder/01-build-go.sh
fi
bash scripts/03-app-go.sh "${my_domain}.${my_zone}" "${PORT}" "${my_project_name}"
