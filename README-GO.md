# libreoffice-as-a-service (LaaS) - Go Version

Convert documents through libreoffice (soffice) as a service

This is a rewrite of the original Node.js/Fastify version in Go for improved simplicity and performance.
...
```bash
git clone git@github.com:paperos-labs/libreoffice-as-a-service.git
...
```bash
bash scripts/install-go.sh dev dev-laas example.com libreoffice-as-a-service
```
...
The Go version maintains the same API and functionality as the original Node.js/Fastify version, with these improvements:

1. **Simpler dependencies**: Uses standard library HTTP router with `PathValue()` for route parameters
2. **Better performance**: Native Go performance without V8 overhead
3. **Single binary**: Compiles to a single executable with no runtime dependencies
4. **Graceful shutdown**: Built-in support for clean server shutdown
5. **Type safety**: Compile-time type checking

The API endpoints and behavior remain identical, so existing clients require no changes.

## Future Plans

- Add pandoc support in a future PR
